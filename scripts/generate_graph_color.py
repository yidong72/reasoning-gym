...
def generate_dataset(local_dir: str, data_size: int = 500, train_frac: float = 0.8, number_of_vertices: int = 10) -> str:
   # Add examples for each dataset
    train_data: List[Dict[str, Any]] = []
    for i, name in enumerate(sorted(DATASETS.keys())):
        if os.path.exists(os.path.join(local_dir, f'{name}_{number_of_vertices}_train.jsonl')):
            print(f"Skipping {name} dataset ({i+1}/{len(DATASETS)})")
            # load train data cache
            with open(os.path.join(local_dir, f'{name}_{number_of_vertices}_train.jsonl'), 'r') as f:
        ...
        if name == 'graph_color':
            ...
            dataset = create_dataset(name, size=initial_size, seed=42, num_colors=4, num_vertices=number_of_vertices, edge_probability=0.4)
        ...
        with open(os.path.join(local_dir, f'{name}_{number_of_vertices}_train.jsonl.tmp'), 'w') as f:
        ...
        with open(os.path.join(local_dir, f'{name}_{number_of_vertices}_test.jsonl.tmp'), 'w') as f:
            for data in test_data:
                f.write(json.dumps(data, ensure_ascii=False) + '\n')
        print(f'saving {name} test data to {local_dir}')
        test_df.to_parquet(os.path.join(local_dir, f'{name}_{number_of_vertices}.parquet'))
    ...
    # train_df.to_parquet(os.path.join(local_dir, f'{name}_{number_of_vertices}.parquet'))
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        raise ValueError("Please provide the number of vertices as an argument")
    # Ensure scripts directory exists
    script_dir = Path(__file__).parent
    local_dir = script_dir.parent / "data"
    if not local_dir.exists():
        local_dir.mkdir(parents=True)
    generate_dataset(local_dir, data_size=500, train_frac=0.8, number_of_vertices=number_of_vertices)
